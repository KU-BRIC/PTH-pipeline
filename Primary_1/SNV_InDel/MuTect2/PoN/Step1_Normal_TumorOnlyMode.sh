####################################################################################################################
####################################################################################################################
# Run MuTect2 on normals in tumor only mode.
# Author: Haiying Kong
...
# log directory:
log_dir=${all_dir}/log/SNV_InDel/MuTect2/PoN_Step1
rm -rf ${log_dir}
mkdir -p ${log_dir}

# error directory:
error_dir=${all_dir}/error/SNV_InDel/MuTect2/PoN_Step1
rm -rf ${error_dir}
mkdir -p ${error_dir}

####################################################################################################################
# Create directory to save results.
rm -rf ${all_dir}/Lock/SNV_InDel/MuTect2/PoN/Normal
mkdir -p ${all_dir}/Lock/SNV_InDel/MuTect2/PoN/Normal
...
do
  ...
  then
    ...
    vcf=${all_dir}/Lock/SNV_InDel/MuTect2/PoN/Normal/${sam}.vcf.gz

    # Submit job.
    qsub -o ${log_dir}/${sam}.log -e ${error_dir}/${sam}.error -N ${batch}_${sam}_MuTect2_PoN_Step1  \
      -v bam=${bam},vcf=${vcf},temp_dir=${temp_dir}  \
      /home/projects/cu_10184/projects/PTH/Code/Primary_1/SNV_InDel/MuTect2/PoN/Step1_Normal_TumorOnlyMode_job.sh
  fi
