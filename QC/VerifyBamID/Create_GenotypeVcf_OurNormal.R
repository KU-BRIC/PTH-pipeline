####################################################################################################################
####################################################################################################################
# Create genotype vcf for each sample with genotype locations decided by our normal samples.
# Author: Haiying Kong
...
snp = read.table('Reference/FASTQuick/OurSample/CommonVariants_Norm.txt', header=FALSE, sep='\t')
snp$V5 = ''
snp = snp[ ,c(1,2,5,3,4)]
names(snp) = paste0('V', 1:5)
...
for (batch in batches)  {
  ...
  for (vcf.file in vcf.files)  {
    ...
    if (n.rows>0)  {
      ...
      aster = merge(snp, vcf, by=names(snp)[-3], all.x=TRUE, all.y=FALSE)
