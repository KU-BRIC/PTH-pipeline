...
mv ${BAM_dir}/${sample}_marked_dup_metrics.txt ${BAM_dir}/lock/
mv ${BAM_dir}/${sample}_recal_data.table ${BAM_dir}/lock/
...
vardict -G $hg -N ${sample} -b ${BAM_dir}/${sample}.bam $target_nochr -c 1 -S 2 -E 3 -t  \
  ...
  | teststrandbias.R \
  | var2vcf_valid.pl -N ${sample} -E -f ${allel_freq} > ${Lock_SNV_InDel_dir}/VarDict/vcf/${sample}.vcf

# Annotation with Funcotator:
gatk Funcotator \
  -R $hg -V ${Lock_SNV_InDel_dir}/VarDict/vcf/${sample}.vcf -O ${Lock_SNV_InDel_dir}/VarDict/maf/${sample}.maf \
...
Rscript /home/projects/cu_10184/projects/PTH/Code/Source/SNV_InDel/Fill_DP_AF_FuncotatorMAF_nonSNVer_job.R ${Lock_SNV_InDel_dir}/VarDict ${sample} 'VarDict'
...
# Run SNVer.
conda activate
snver -r $hg -i ${BAM_dir}/${sample}.bam -o ${Lock_SNV_InDel_dir}/SNVer/vcf/${sample}  \
...
# Annotation with Funcotator:
gatk Funcotator \
  -R $hg -V ${Lock_SNV_InDel_dir}/SNVer/vcf/${sample}.filter.vcf -O ${Lock_SNV_InDel_dir}/SNVer/maf/${sample}.snv.maf \
  --output-file-format MAF --ref-version hg38 --data-sources-path ${Funcotator_DB} \
  --disable-sequence-dictionary-validation
gatk Funcotator \
  -R $hg -V ${Lock_SNV_InDel_dir}/SNVer/vcf/${sample}.indel.filter.vcf -O ${Lock_SNV_InDel_dir}/SNVer/maf/${sample}.indel.maf \
  --output-file-format MAF --ref-version hg38 --data-sources-path ${Funcotator_DB} \
  --disable-sequence-dictionary-validation

# Fill missing DP and AF in Funcotator maf output.
Rscript /home/projects/cu_10184/projects/PTH/Code/Source/SNV_InDel/Fill_DP_AF_FuncotatorMAF_SNVer_job.R ${Lock_SNV_InDel_dir}/SNVer ${sample} 'SNVer'
...
lofreq call-parallel --pp-threads ${n_thread} --call-indels -f $hg -l ${target_chr}  \
  ...
  -o ${Lock_SNV_InDel_dir}/LoFreq/vcf/${sample}.vcf ${BAM_dir}/${sample}.bam

# Annotation with Funcotator:
gatk Funcotator \
  -R $hg -V ${Lock_SNV_InDel_dir}/LoFreq/vcf/${sample}.vcf -O ${Lock_SNV_InDel_dir}/LoFreq/maf/${sample}.maf \
  --output-file-format MAF --ref-version hg38 --data-sources-path ${Funcotator_DB} \
  --disable-sequence-dictionary-validation

# Fill missing DP and AF in Funcotator maf output.
Rscript /home/projects/cu_10184/projects/PTH/Code/Source/SNV_InDel/Fill_DP_AF_FuncotatorMAF_nonSNVer_job.R ${Lock_SNV_InDel_dir}/LoFreq ${sample} 'LoFreq'
...
####################################################################################################################
# Long:
Rscript /home/projects/cu_10184/projects/PTH/Code/Source/SNV_InDel/Filter/Filter_Variants.R ${Result_SNV_InDel_dir} ${sample} "Long"

# Medium:
Rscript /home/projects/cu_10184/projects/PTH/Code/Source/SNV_InDel/Filter/Filter_Variants.R ${Result_SNV_InDel_dir} ${sample} "Medium"

# Short:
Rscript /home/projects/cu_10184/projects/PTH/Code/Source/SNV_InDel/Filter/Filter_Variants.R ${Result_SNV_InDel_dir} ${sample} "Short"
...
rm -rf ${Lock_CNV_dir}/CNACS/${sample}
mkdir -p ${Lock_CNV_dir}/CNACS/${sample}

toil_cnacs run \
    ${Lock_CNV_dir}/CNACS/${sample}/jobstore/ \
    --stats \
    --db_dir /home/projects/cu_10184/projects/PTH/Reference/CNACS/db \
    --outdir ${Lock_CNV_dir}/CNACS \
    --pool_dir /home/projects/cu_10184/projects/PTH/Reference/CNACS/PoN \
    --fasta $hg \
    --samp ${BAM_dir}/${sample}.bam

rm -r ${Lock_CNV_dir}/CNACS/${sample}/tmp
rm -r ${Lock_CNV_dir}/CNACS/${sample}/jobstore
...
cnvkit.py batch ${BAM_dir}/${sample}.bam \
    ...
    --output-dir ${Lock_CNV_dir}/CNVkit

echo "cnvkit.py scatter ${Lock_CNV_dir}/CNVkit/${sample}.cnr -s ${Lock_CNV_dir}/CNVkit/${sample}.cns -o ${Lock_CNV_dir}/CNVkit/${sample}.scatter.pdf"

cnvkit.py scatter ${Lock_CNV_dir}/CNVkit/${sample}.cnr -s ${Lock_CNV_dir}/CNVkit/${sample}.cns -o ${Lock_CNV_dir}/CNVkit/${sample}.scatter.pdf
cnvkit.py diagram ${Lock_CNV_dir}/CNVkit/${sample}.cnr -s ${Lock_CNV_dir}/CNVkit/${sample}.cns -o ${Lock_CNV_dir}/CNVkit/${sample}.diagram.pdf
...
################################################
Rscript /home/projects/cu_10184/projects/PTH/Code/Source/ITD/${itd_scheme}/Combine_Filter.R ${batch} ${sample} ${Lock_ITD_dir} ${Result_ITD_dir} ${itd_thresh_n_alt}
