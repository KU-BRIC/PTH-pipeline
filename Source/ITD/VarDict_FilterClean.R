...
if (nrow(maf)>0)  {
  ...
  maf = maf[ ,c(1,2,4,5,17,12,11)]
  names(maf) = c('Batch', 'Sample', 'Chrom', 'Start', 'N_Alt', 'DP', 'AF')
  maf = maf[ ,c('Batch', 'Sample', 'Chrom', 'Start', 'N_Alt', 'DP', 'AF')]
  maf = maf[order(maf$Batch,maf$Sample,maf$Chrom,maf$Start), ]
  maf = aggregate(cbind(Start,N_Alt,DP,AF) ~ Batch+Sample+Chrom, data=maf, list)
  ...
  for (i in 1:nrow(maf))  {
    start = unlist(maf$Start[i])
    n_alt = unlist(maf$N_Alt[i])
    dp = unlist(maf$DP[i])
    af = unlist(maf$AF[i])
    if (length(start)==1)  {
