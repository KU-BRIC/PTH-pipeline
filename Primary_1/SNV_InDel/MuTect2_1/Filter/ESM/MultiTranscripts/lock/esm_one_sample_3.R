...
# Save non-SAAS.
write.table(maf[-idx, ], paste0(esm.dir, '/', sam, '_rest.maf'), row.names=FALSE, col.names=TRUE, quote=FALSE, sep='\t')
...
header = t(header)
write.table(header, paste0(esm.dir, '/', sam, '_SAAS.maf'), row.names=FALSE, col.names=FALSE, quote=FALSE, sep='\t')
...
for (i in 1:nrow(maf))  {
  # 
  granges = maf[i, c('Chromosome', 'Start_Position', 'End_Position', 'Strand')]
  names(granges) = c('chr', 'start', 'end', 'strand')
  granges = makeGRangesFromDataFrame(granges)
  allele = DNAStringSet(maf$Tumor_Seq_Allele2[i])
  coding = predictCoding(query=granges, subject=txdb, seqSource=Hsapiens, varAllele=allele)
  coding

  # If there is no coding change, go to next iteration.
  if (length(coding)==0)  next

  #
  p.seq = proteins(edb, filter = TxIdFilter('75658'),
