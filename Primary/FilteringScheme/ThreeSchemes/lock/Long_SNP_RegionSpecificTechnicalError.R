...
filter.style = 'Long'
thresh.maf = 0.02
thresh.silhouette = 0.9
thresh.lower.cluster.center = 0.05

####################################################################################################################
####################################################################################################################
# Define directory to save results.
ref.dir = paste0('/home/projects/cu_10184/projects/PTH/Reference/Filtering/', filter.style)
...
####################################################################################################################
# Find column index for normal and patients.
...
ans = mclapply(1:nrow(AF), function(i)  {
                             ...
                             if (nrow(af.non0)>=5 & sd(af.non0$AF)!=0)  {
                               ...
                               if (silhouette>thresh.silhouette)  {
                                 # k-means clustering.
                                 km = kmeans(af.non0, 2)
                                 centers = km$centers 
                                 ...
                                 if (centers[2]<thresh.lower.cluster.center)  {
...
centers.1 = sapply(ans, function(x) list(x[[1]][[2]]))
centers.2 = sapply(ans, function(x) list(x[[1]][[3]]))

idx = which(flag != 0)
apple = cbind(AF[idx, 1:6], unlist(centers.1[idx]), unlist(centers.2[idx]))
