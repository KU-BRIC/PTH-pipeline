####################################################################################################################
####################################################################################################################
# Create a table for list of splicing sites from all samples.
# Author: Haiying Kong
...
for (batch in batches)  {
  ...
  for (maf.file in maf.files)  {
    ...
    maf = maf[maf$Variant_Classification=='Splice_Site', ]
    if (nrow(maf)==0)  next
...
# Save the results.
write.table(apple, '/home/projects/cu_10184/projects/PTH/AllBatches_1/Result/SNV_InDel/MuTect2_1/MMSplice/SpliceSite.maf',
            row.names=FALSE, col.names=TRUE, quote=FALSE, sep='\t')

# Find and save unique splice site list.
apple = unique(apple[ ,c(3:8,10)])
write.table(apple, '/home/projects/cu_10184/projects/PTH/AllBatches_1/Result/SNV_InDel/MuTect2_1/MMSplice/Unique_SpliceSite.txt',
            row.names=FALSE, col.names=TRUE, quote=FALSE, sep='\t')
