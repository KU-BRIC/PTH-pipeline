...
# log directory:
log_dir=${batch_dir}/log/SNV_InDel/MuTect2_1/Filter/${scheme_name}/MultiTranscripts
rm -rf ${log_dir}
mkdir -p ${log_dir}

# error directory:
error_dir=${batch_dir}/error/SNV_InDel/MuTect2_1/Filter/${scheme_name}/MultiTranscripts
rm -rf ${error_dir}
mkdir -p ${error_dir}
...
####################################################################################################################
rm -rf ${batch_dir}/Result/SNV_InDel/MuTect2_1/${scheme_name}/MultiTranscripts
mkdir -p ${batch_dir}/Result/SNV_InDel/MuTect2_1/${scheme_name}/MultiTranscripts/Original
mkdir -p ${batch_dir}/Result/SNV_InDel/MuTect2_1/${scheme_name}/MultiTranscripts/temp
...
for sam in ${samples[@]}
do
  qsub -o ${log_dir}/${sam}.log -e ${error_dir}/${sam}.error -N ${batch}_${sam}_${scheme_name}_MultiTranscripts  \
    -l nodes=1:ppn=8:gpus=1,mem=20gb,walltime=20:00:00  \
    -v dir_name=${dir_name},batch=${batch},sam=${sam},temp_dir=${temp_dir}  \
    /home/projects/cu_10184/projects/PTH/Code/Primary_1/SNV_InDel/MuTect2_1/Filter/${scheme_name}/MultiTranscripts/ESM_job.sh
done
